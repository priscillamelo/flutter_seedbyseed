...
class LotHelper {
  ...
  Future<List> getAllLots(int idGerminationTest) async {
    final Database database = await _databaseApp.getDatabase;
    final List listMapLots = await database.query(tableName, where: '${LotConst.kIDGERMINATIONTESTFOREIGNKEY} = ?', whereArgs: [idGerminationTest]);
  ...
  Future<List> getLot(int idGerminationTest, int idLot) async {
    ...
    final List listLot = await database.query(
      tableName,
      limit: 1,
      where: '${LotConst.kIDGERMINATIONTESTFOREIGNKEY} = ? AND ${LotConst.kIDLOTCOLUMN} = ?',
  ...
  Future<Lot?> getDailyCount(int idGerminationTest, int idLot) async {
    ...
    if (listLot.isNotEmpty) {
      ...
      return Lot.fromMap(map)..dailyCount = jsonDecode(map[LotConst.kDAILYCOUNTCOLUMN]);
    }
